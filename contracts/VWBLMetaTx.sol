...
abstract contract VWBLProtocol is ERC721Enumerable, IERC2981 {
    ...
    function _mint(address _msgSender, string memory _getKeyURl, uint256 _royaltiesPercentage, bytes32 _documentId) internal returns (uint256) {
 ...
 */
contract VWBL is VWBLProtocol, Ownable, IVWBL, BaseRelayRecipient {
     ...
     */
    function mint(string memory _getKeyURl, uint256 _royaltiesPercentage, bytes32 _documentId) public returns (uint256) {
        uint256 tokenId = super._mint(_msgSender(), _getKeyURl, _royaltiesPercentage, _documentId);
